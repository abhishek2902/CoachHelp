...
UnitTest.asynctest('PatchFileInputTest', (success, failure) => {
    ...
    const filesState = Singleton.value();
    const pickFiles = (body, next) => {
        const elm = SugarElement.fromHtml('<input type="file">');
        elm.dom.onchange = () => {
            Remove.remove(elm);
            next(elm.dom.files);
        };
    ...
    const cPickFiles = Chain.async((input, next, _die) => pickFiles(input, next));
