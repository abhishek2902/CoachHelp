...
import { Attribute, Compare, Css, Insert, PredicateFilter, Remove, SelectorFilter, SugarBody, SugarNode, SugarText, Traverse, Truncate } from '@ephox/sugar';
...
UnitTest.test('Arbitraries Test', () => {
    ...
    checkProperty('Text nodes should have node type 3', Arbitraries.content('netext'), (textnode) => {
        Assertions.assertEq('Node type of "netext"', 3, SugarNode.type(textnode));
        return true;
    });
    ...
    }), (data) => assertProperty('style and attr api', data, (elem) => SugarNode.isTag('span')(elem) && (Attribute.get(elem, 'data-a') !== 'b' || Css.getRaw(elem, 'color').getOr('') !== 'red')));
    checkProperty('Testing out attribute and style decorators', Arbitraries.content('test-data', {
        ...
        return (!(hasDataCustom && hasContentEditable) && (hasDataCustom || hasContentEditable)) && (!(hasColor && hasVisibility) && (hasColor || hasVisibility)) && (Traverse.firstChild(leaf).isNone());
    });
    ...
    assertProperty('(display === inline)', element, (elem) => SugarNode.isElement(elem) && Css.get(elem, 'display') !== 'inline' || Arr.contains(['span-underline', 'span-strikethrough'], SugarNode.name(elem))));
    ...
    checkProperty('Formatting elements should only contain (display === inline)', Arbitraries.content('formatting'), (section) => assertProperty('nothing should have display block inside a formatting element', section, (elem) => !Compare.eq(section, elem) && SugarNode.isElement(elem) && Css.get(elem, 'display') !== 'inline'));
...
//# sourceMappingURL=ArbitrariesTest.js.map