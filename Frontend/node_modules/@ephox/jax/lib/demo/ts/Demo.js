...
Http.get({
    ...
    result.fold(() => console.log('error'), (json) => {
...
Http.del({
    ...
    result.fold(() => console.log('error'), (text) => {
...
Http.get({
    ...
    result.fold(() => console.log('error'), (blob) => {
...
Http.download({
    ...
    result.fold((e) => console.log('download blob error', e), (blob) => {
        console.log('download blob', blob);
    });
});
const jwtFactory = (_foo) => FutureResult.pure('123');
HttpJwt.put({
    url: 'https://httpbin.org/put',
    body: HttpData.jsonData({ important: 'data' }),
    responseType: DataType.JSON
