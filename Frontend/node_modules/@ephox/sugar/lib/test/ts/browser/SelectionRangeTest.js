...
UnitTest.test('WindowSelectionTest', () => {
    ...
    const find = (path) => Hierarchy.follow(container, path).getOrDie('invalid path');
    const detection = PlatformDetection.detect();
    const detector = (variants) => {
    ...
    const checkSelection = (label, variants, start, finish) => {
    ...
    const checkUniCodeSelection = (content) => {
        Remove.empty(container);
        Html.set(container, content);
        return checkSelection;
    };
    const checkStringAt = (label, expectedStr, start, finish) => {
    ...
    }, Situ.on(find([0, 1, 0]), 'w'.length), Situ.on(find([0, 1, 0]), 'wo'.length));
    ...
    }, Situ.on(find([0, 1, 0]), 'wo'.length), Situ.on(find([0, 1, 0]), 'w'.length));
    ...
    }, Situ.before(find([0, 2])), Situ.on(find([0, 1, 0]), 'w'.length));
    ...
    }, Situ.on(find([0]), 0), Situ.on(find([0]), 7));
    ...
    }, Situ.on(find([0]), 7), Situ.on(find([0]), 0));
    ...
    }, Situ.after(find([0, 3, 1, 0])), Situ.before(find([0, 6])));
    ...
    }, Situ.before(find([0, 6])), Situ.after(find([0, 3, 1, 0])));
    ...
    'This world is not what I', Situ.on(find([0]), 0), Situ.on(find([0]), 7));
    checkStringAt('RTL SimSelection (This world is not what I)', 'This world is not what I', Situ.on(find([0]), 7), Situ.on(find([0]), 0));
    ...
    }, Situ.on(find([0]), 0), Situ.on(find([0]), 0));
