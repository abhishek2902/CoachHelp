...
UnitTest.test('Browser Test: SelectionTest', () => {
    ...
    const assertFragmentHtml = (expected, fragment) => {
    ...
    WindowSelection.deleteAt(window, SimSelection.exact(Hierarchy.follow(p1, [0, 0]).getOrDie('looking for text in a'), 'li'.length, Hierarchy.follow(p1, [1, 0]).getOrDie('looking for text in span'), 'wor'.length));
    Assert.eq('', '<a>li</a><span>d</span>', Html.get(p1));
    Remove.remove(p1);
    Remove.remove(p2);
    const assertRng = (selection, expStart, expSoffset, expFinish, expFoffset) => {
