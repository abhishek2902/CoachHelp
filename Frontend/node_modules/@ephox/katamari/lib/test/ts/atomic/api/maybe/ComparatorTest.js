    ...
    context('is', () => {
        it('is always false for "Nothing"', () => {
            fc.assert(fc.property(fc.anything(), (thing) => {
                const matches = Fun.pipe(Maybes.nothing(), Maybes.is(thing));
                assert.isFalse(matches);
            }));
        });
        it('is always false for "Nothing" with explicit comparator', () => {
            fc.assert(fc.property(fc.anything(), (thing) => {
                const matches = Fun.pipe(Maybes.nothing(), Maybes.is(thing, Fun.die('should not be called')));
                assert.isFalse(matches);
            }));
        });
        it('is correct for "Just"', () => {
            fc.assert(fc.property(fc.string(), (thing) => {
                const matches = Fun.pipe(Maybes.just(thing), Maybes.is(thing));
                assert.isTrue(matches);
            }));
            fc.assert(fc.property(fc.string(), (thing) => {
                const matches = Fun.pipe(Maybes.just(thing), Maybes.is(thing + 'foo'));
                assert.isFalse(matches);
            }));
        });
        it('is correct for "Just" with an explicit comparator', () => {
            // Just an example data type that doesn't have a simple equality
            const thunkEq = (a, b) => a() === b();
            fc.assert(fc.property(fc.string(), (thing) => {
                const matches = Fun.pipe(Maybes.just(Fun.constant(thing)), Maybes.is(Fun.constant(thing), thunkEq));
                assert.isTrue(matches);
            }));
            fc.assert(fc.property(fc.string(), (thing) => {
                const matches = Fun.pipe(Maybes.just(Fun.constant(thing)), Maybes.is(() => thing + 'foo', thunkEq));
                assert.isFalse(matches);
            }));
        });
    });
...
//# sourceMappingURL=ComparatorTest.js.map