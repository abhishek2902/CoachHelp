...
describe('atomic.katamari.api.arr.ArrMapTest', () => {
    ...
    context('functor laws', () => {
        it('obeys identity law', () => {
            fc.assert(fc.property(fc.array(fc.nat()), (xs) => assert.deepEqual(Arr.map(xs, Fun.identity), xs)));
        });
        it('obeys associative law', () => {
            const f = plus3;
            const g = dbl;
            fc.assert(fc.property(fc.array(fc.nat()), (xs) => assert.deepEqual(Arr.map(Arr.map(xs, g), f), Arr.map(xs, Fun.compose(f, g)))));
        });
    });
});
