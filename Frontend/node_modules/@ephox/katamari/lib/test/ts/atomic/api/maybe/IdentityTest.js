...
describe('atomic.katamari.maybe.IdentityTest', () => {
    ...
    it('allows folding on "Just" maybes', () => {
        ...
        Fun.pipe(Maybes.just('test'), Maybes.fold(Fun.die('Should call other branch'), (test) => {
            assert.equal(test, 'test');
            return 'other-test';
        }), (result) => assert.equal(result, 'other-test'));
        // Complex values, simple assertions
        fc.assert(fc.property(fc.anything(), (thing) => {
            Fun.pipe(Maybes.just(thing), Maybes.fold(Fun.die('Should call other branch'), Fun.noop));
        }));
    });
    it('allows folding on "Nothing" maybes', () => {
        Fun.pipe(Maybes.nothing(), Maybes.fold((...args) => {
            assert.isEmpty(args);
            return 'other-test';
        }, Fun.die('Should call other branch')), (result) => assert.equal(result, 'other-test'));
    });
});
//# sourceMappingURL=IdentityTest.js.map