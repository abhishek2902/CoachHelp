...
UnitTest.test('All valid floats are valid', () => {
    ...
    fc.float(), 
    // big floats
    fc.tuple(fc.float(), fc.integer(-20, 20)).map(([mantissa, exponent]) => mantissa * Math.pow(10, exponent))), (num) => {
...
UnitTest.test('Accepts known units', () => {
    Assert.succeeds('Accepts % in relative', () => Optionals.is(Dimension.parse('1%', ['relative']), { value: 1, unit: '%' }));
    Assert.succeeds('Accepts px in fixed', () => Optionals.is(Dimension.parse('20px', ['fixed']), { value: 20, unit: 'px' }));
    Assert.succeeds('Does not accept % in fixed', () => Dimension.parse('1%', ['fixed']).isNone());
    Assert.succeeds('Accepts px in fixed/relative', () => Optionals.is(Dimension.parse('20px', ['fixed', 'relative']), { value: 20, unit: 'px' }));
    Assert.succeeds('Accepts unitless in unitless', () => Optionals.is(Dimension.parse('1.4', ['empty']), { value: 1.4, unit: '' }));
