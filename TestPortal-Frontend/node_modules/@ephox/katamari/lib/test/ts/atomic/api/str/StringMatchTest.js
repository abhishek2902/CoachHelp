...
describe('atomic.katamari.api.str.StringMatchTest', () => {
    it('unit tests', () => {
        const check = (testcase) => {
            ...
            assert.equal(StringMatch.matches(StringMatch.not(testcase.match), testcase.input), !testcase.expected);
        };
        const testcases = [
    ...
    it('StringMatch.matches(StringMatch.starts(s1), s1 + s) === true', () => {
        fc.assert(fc.property(fc.string(), fc.string(), (s, s1) => {
            assert.isTrue(StringMatch.matches(StringMatch.starts(s1, StringMatch.caseInsensitive), s1 + s));
        }));
    });
    it('StringMatch.matches(StringMatch.contains(s1), s1 + s) === true', () => {
        fc.assert(fc.property(fc.string(), fc.string(), (s, s1) => {
            assert.isTrue(StringMatch.matches(StringMatch.contains(s1, StringMatch.caseInsensitive), s1 + s));
        }));
    });
    it('StringMatch.matches(StringMatch.contains(s1), s + s1) === true', () => {
        fc.assert(fc.property(fc.string(), fc.string(), (s, s1) => {
            assert.isTrue(StringMatch.matches(StringMatch.contains(s1, StringMatch.caseInsensitive), s + s1));
        }));
    });
    it('StringMatch.matches(StringMatch.contains(s1), s) === s.indexOf(s1)', () => {
        fc.assert(fc.property(fc.asciiString(), fc.asciiString(1, 40), (s, s1) => {
            assert.equal(StringMatch.matches(StringMatch.contains(s1, StringMatch.caseInsensitive), s), s.toLowerCase().indexOf(s1.toLowerCase()) > -1);
        }));
    });
    it('StringMatch.matches(StringMatch.contains(s), s) === true', () => {
        fc.assert(fc.property(fc.string(), (s) => {
            assert.isTrue(StringMatch.matches(StringMatch.contains(s, StringMatch.caseInsensitive), s));
        }));
    });
    it('StringMatch.matches(StringMatch.exact(s), s + s1) === false', () => {
        fc.assert(fc.property(fc.string(1, 40), fc.string(1, 40), (s, s1) => {
            assert.isFalse(StringMatch.matches(StringMatch.exact(s, StringMatch.caseInsensitive), s + s1));
        }));
    });
    it('StringMatch.matches(StringMatch.exact(s), s1 + s) === false', () => {
        fc.assert(fc.property(fc.string(1, 40), fc.string(1, 40), (s, s1) => {
            assert.isFalse(StringMatch.matches(StringMatch.exact(s, StringMatch.caseInsensitive), s1 + s));
        }));
    });
    it('StringMatch.matches(StringMatch.exact(s), s) === true', () => {
        fc.assert(fc.property(fc.string(1, 40), (s) => {
            assert.isTrue(StringMatch.matches(StringMatch.exact(s, StringMatch.caseInsensitive), s));
        }));
    });
    it('StringMatch.matches(StringMatch.exact(s), s) === false when different case and case-insensitive', () => {
        fc.assert(fc.property(fc.asciiString(1, 40), (s) => s.toUpperCase() === s.toLowerCase() || StringMatch.matches(StringMatch.exact(s.toLowerCase(), StringMatch.caseInsensitive), s.toUpperCase())));
    });
    it('StringMatch.matches(StringMatch.exact(s), s) === false when different case and case-sensitive', () => {
        fc.assert(fc.property(fc.asciiString(1, 40), (s) => s.toUpperCase() === s.toLowerCase() || !StringMatch.matches(StringMatch.exact(s.toLowerCase(), StringMatch.caseSensitive), s.toUpperCase())));
    });
    it('StringMatch.matches(StringMatch.all(s1), *) === true', () => {
        fc.assert(fc.property(fc.string(), fc.string(), (s) => {
            assert.isTrue(StringMatch.matches(StringMatch.all(), s));
        }));
    });
});
