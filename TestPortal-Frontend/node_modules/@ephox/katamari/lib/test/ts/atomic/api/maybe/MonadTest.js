...
describe('atomic.katamari.maybe.MonadTest', () => {
    context('flatten', () => {
        ...
        it('mirrors bind', () => {
            ...
            fc.assert(fc.property(arbNested, (maybe) => {
                ...
                const bound = Maybes.bind(Fun.identity)(maybe);
    ...
    context('bind', () => {
        it('does not call the bind function when binding on Nothing', () => {
            const nothing = Fun.pipe(Maybes.nothing(), Maybes.bind(Fun.die('Should not call binder as there is nothing to bind')));
        ...
        it('passes the right values through the bind function when binding on Just', () => {
            const just = Fun.pipe(Maybes.just('test'), Maybes.bind((test) => {
            ...
            const nothing = Fun.pipe(Maybes.just('test'), Maybes.bind((test) => {
                assert.equal(test, 'test');
                return Maybes.nothing();
            }));
            assert.isTrue(Maybes.isNothing(nothing), 'Expected nothing');
    ...
    context('bindNullable', () => {
        it('does not call the bind function when binding on Nothing', () => {
            const nothing = Fun.pipe(Maybes.nothing(), Maybes.bindNullable(Fun.die('Should not call binder as there is nothing to bind')));
            assert.isTrue(Maybes.isNothing(nothing), 'Expected nothing');
        });
        it('passes the right values through the bind function when binding on Just', () => {
            const nothing1 = Fun.pipe(Maybes.just('test'), Maybes.bindNullable((test) => {
                assert.equal(test, 'test');
                return null;
            }));
            assert.isTrue(Maybes.isNothing(nothing1));
            const nothing2 = Fun.pipe(Maybes.just('test'), Maybes.bindNullable((test) => {
                assert.equal(test, 'test');
                return undefined;
            }));
            assert.isTrue(Maybes.isNothing(nothing2));
            const just = Fun.pipe(Maybes.just('test'), Maybes.bindNullable((test) => {
                assert.equal(test, 'test');
                return 'test2';
            }));
            if (Maybes.isJust(just)) {
