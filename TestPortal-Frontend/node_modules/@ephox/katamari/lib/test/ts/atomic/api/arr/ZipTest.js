...
describe('atomic.katamari.api.arr.ZipTest', () => {
    it('unit tests', () => {
        const check1 = (expectedZipToObject, expectedZipToTuples, keys, values) => {
            const sort = (a, ord) => {
                const c = a.slice();
                c.sort(ord);
                return c;
        ...
        check1(Optional.some({ q: 'a', r: 'x' }), Optional.some([{ k: 'q', v: 'a' }, { k: 'r', v: 'x' }]), ['q', 'r'], ['a', 'x']);
        check1(Optional.some({}), Optional.some([]), [], []);
        check1(Optional.none(), Optional.none(), [], ['x']);
        check1(Optional.none(), Optional.none(), [], ['x', 'y']);
        check1(Optional.none(), Optional.none(), ['q'], []);
        check1(Optional.none(), Optional.none(), ['q', 'r'], []);
        check1(Optional.none(), Optional.none(), ['q', 'r'], ['a']);
    });
    it('returns matching keys and values', () => {
        fc.assert(fc.property(fc.array(fc.asciiString(1, 30)), (rawValues) => {
    ...
    it('matches corresponding tuples', () => {
        fc.assert(fc.property(fc.array(fc.integer()), fc.array(fc.integer()), (keys, values) => {
