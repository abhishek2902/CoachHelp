...
const modifyStartedEntryTo = (entries, f) => Arr.last(entries).fold(() => entries, (lastEntry) => {
...
const modifyStartedEntry = (logs, f) => ({
    history: modifyStartedEntryTo(logs.history, f)
});
const modifyLastEntryTo = (entries, f) => 
...
export const addLogEntry = (logs, message) => {
    if (DISABLE_LOGGING) {
        return logs;
    }
    const newEntry = {
...
export const pushLogLevel = (logs) => {
    if (DISABLE_LOGGING) {
        return logs;
    }
...
export const addStackTrace = (logs, err) => {
    if (DISABLE_LOGGING) {
        return logs;
    }
    return modifyLastEntry(logs, (entry) => ({
...
const init = () => initLogsWith([]);
const initLogsWith = (history) => ({
    history
});
const concat = (logs1, logs2) => initLogsWith(Arr.flatten([logs1.history, logs2.history]));
const single = (message) => addLogEntry(init(), message);
export const TestLogs = {
    getOrInit: (logs) => logs !== undefined ? logs : initLogsWith([]),
    init,
    initLogsWith,
    concat,
    addLogEntry,
