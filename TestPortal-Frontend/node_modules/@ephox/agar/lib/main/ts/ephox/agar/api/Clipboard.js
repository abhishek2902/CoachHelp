...
const pasteDataTransfer = (target, mutator) => {
...
const pasteItems = (target, items) => pasteDataTransfer(target, (dataTransfer) => {
    Obj.each(items, (data, mime) => {
        dataTransfer.setData(mime, data);
    });
});
const pasteFiles = (target, files) => pasteDataTransfer(target, (dataTransfer) => {
    Arr.each(files, (file) => {
        dataTransfer.items.add(file);
    });
});
const cPasteDataTransfer = (mutator) => Chain.op((target) => pasteDataTransfer(target, mutator));
const cPasteItems = (items) => Chain.op((target) => pasteItems(target, items));
const cPasteFiles = (files) => Chain.op((target) => pasteFiles(target, files));
const sPasteDataTransfer = (mutator, selector) => Chain.isolate({}, ChainSequence.sequence([
    Chain.injectThunked(SugarBody.body),
    cFindIn(selector),
    cPasteDataTransfer(mutator)
]));
const sPasteItems = (items, selector) => Chain.isolate({}, ChainSequence.sequence([
    Chain.injectThunked(SugarBody.body),
    cFindIn(selector),
    cPasteItems(items)
]));
const sPasteFiles = (files, selector) => Chain.isolate({}, ChainSequence.sequence([
    Chain.injectThunked(SugarBody.body),
    cFindIn(selector),
    cPasteFiles(files)
]));
