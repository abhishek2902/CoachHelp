...
UnitTest.asynctest('WaiterChainTest', (success, failure) => {
    const makeTryUntilChain = (label, interval, amount) => {
        let counter = 0;
        return Waiter.cTryUntil(label + ': TryUntil counter', Chain.on((_value, next, die, logs) => {
            counter++;
            if (counter === 5) {
    ...
    const makeTryUntilNotChain = (label, interval, amount) => {
        let counter = 0;
        return Waiter.cTryUntilNot(label + ': TryUntilNot counter', Chain.on((_value, next, die, logs) => {
            counter++;
            if (counter < 10) {
    ...
    const makeDelayChain = (label, timeout, delay) => Waiter.cTimeout(label + ': Waiter timeout', Chain.on((_value, next, die, logs) => {
        setTimeout(() => {
            next(_value, logs);
        }, delay);
    }), timeout);
