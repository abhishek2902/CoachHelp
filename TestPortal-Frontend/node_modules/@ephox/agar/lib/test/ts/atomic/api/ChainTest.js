...
UnitTest.asynctest('ChainTest', (success, failure) => {
    const cIsEqual = (expected) => Chain.async((actual, next, die) => {
        if (expected === actual) {
            next(actual);
        }
        else {
    ...
    const cIsEqualAndChange = (expected, newValue) => Chain.async((actual, next, die) => {
        if (expected === actual) {
            next(newValue);
        }
        else {
    ...
    const testChainingFails = StepAssertions.testStepsFail('Cat is not a dog', [
        Chain.asStep({}, [
            Chain.inject('dog'),
            cIsEqual('cat')
        ])
    ...
    const testChainingFailsBecauseChanges = StepAssertions.testStepsFail('Cat is not a dog', [
        Chain.asStep('value', [
            Chain.inject('cat'),
    ...
    const testChainParentAcc = StepAssertions.testChain('Sentence: ', Chain.fromParent(Chain.inject('Sentence: '), [
        acc('T'),
        acc('h'),
        acc('i'),
        acc('s')
    ]));
    const testChainAcc = StepAssertions.testChain('Sentence: This', Chain.fromChainsWith('Sentence: ', [
        acc('T'),
        acc('h'),
        acc('i'),
        acc('s')
    ]));
    const testIsolatedChainAcc = StepAssertions.testChain('Sentence: ', Chain.fromChains([
    ...
    const testChainExists = StepAssertions.testChain('Sentence: This', Chain.fromChains([
    ...
    const testChainExistsFail = StepAssertions.testChainFail('final fail', {}, Chain.exists([
    ...
    const testChainAsync = StepAssertions.testChain('async works!', Chain.async((_value, next) => {
        next('async works!');
    }));
    const testChainAsyncFail = StepAssertions.testChainFail('async fails!', {}, Chain.async((_value, _next, die) => {
        die('async fails!');
    }));
    const testChainAsyncChain = StepAssertions.testChain('async chains!', Chain.fromChains([
        Chain.inject('async chains!'),
        Chain.async((value, next, _die) => {
            next(value);
        })
    ]));
    const testChainRunStepsOnValue = StepAssertions.testChain('runSteps*runStepsOnValue=succ!', Chain.fromChains([
        Chain.inject('runSteps'),
        Chain.runStepsOnValue((s) => [
    ...
    return Pipeline.async({}, [
        ...
        Logger.t('[When using fromIsolatedChains, chains do accumulate when passing, but return the original value]\n', testIsolatedChainAcc),
        Logger.t('[When using exists, ensure that the first successful value is returned]\n', testChainExists),
        Logger.t('[When using exists, if all chains fail, ensure that the last failure is returned]\n', testChainExistsFail),
...
UnitTest.asynctest('Chain.predicate true', (success, failure) => {
    Pipeline.async('stepstate', [
        StepAssertions.testStepsPass('stepstate', [Chain.asStep('chicken', [Chain.predicate((x) => x === 'chicken')])])
    ], () => success(), failure);
});
UnitTest.asynctest('Chain.predicate false', (success, failure) => {
    Pipeline.async('stepstate', [
        StepAssertions.testStepsFail('predicate did not succeed', [Chain.asStep('chicken', [Chain.predicate((x) => x === 'frog')])])
    ], () => success(), failure);
});
